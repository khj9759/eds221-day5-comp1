...
# The line below: create the sequence of year
time_yr <- seq(from = 0, to = 8, by = 1)

# The line below...matrix filled with 4 X 8 
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))

# The line below...recall the first row. 
proj_year[1, ] <- fish_year0

# The for loop below...previous row goes to 4X4 matrix 
for (i in 2:length(time_yr)) {
  
[i,] <- fish_leslie %*% proj_year[i-1,]
}
...
```{r}
demo_longer -> demo %>%
  pivot_longer(clos = c(cats, dogs), names_to = "species", values_to = "count")
```
